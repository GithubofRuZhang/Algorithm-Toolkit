...
load('C_18_Feb_2024_19_40_32.mat')

data = G_out_data.data_process;  % Processed data after overall processing G_out_data.data_O original data
jiangwei_num = G_out_data.jiangwei_num;  % Set the dimensionality reduction number
...
xlabel('x'); ylabel('y')
label_distance = G_out_data.label_distance;  % Determine the distance used for clustering
cluster_num = G_out_data.cluster_num;  % Determine the number of clusters
Z1 = linkage(Z, 'complete');  % Hierarchical clustering
index = cluster(Z1, 'Maxclust', cluster_num);
sc_coefficient = mean(silhouette(data, index'));  % Silhouette coefficient
a = unique(index); % Find the number of categories
yang_num = length(a);
C = cell(1, length(a));
for i = 1:length(a)
...
kuang_width1 = G_out_data.kuang_width;  % Font thickness
...
kuang = G_out_data.kuang;  % Box selection
grid1 = G_out_data.grid;  % Grid selection
zlabel1 = G_out_data.zlabel;  % ylabel
leg_kuang1 = G_out_data.leg_kuang;  % Legend box
kuang_width1 = G_out_data.kuang_width;  % Overall frame setting

for i = 1:yang_num
    data_get = data1(C{1, i}, :);
    plot(data_get(:, 1), data_get(:, 2), fu_str{1, i}, 'Color', color_all(i, :), 'LineWidth', Line_Width(1), 'MarkerSize', makesize); hold on;
end
...
set(gca, 'FontName', FontName1, 'FontSize', FontSize, 'LineWidth', kuang_width1)
xticklabels(get_legend_str)
...
for NN1 = 2:cluster_max
    Z1 = linkage(Z, 'complete');  % Hierarchical clustering
    index = cluster(Z1, 'Maxclust', NN1);

    sc_coefficient1(NN1-1) = mean(silhouette(data, index'));
end

disp_str = ['Cluster number from 2 to ', num2str(cluster_max), ' with silhouette coefficients respectively:'];
disp(disp_str)
disp(sc_coefficient1)
figure
yang_fu3 = {'--p', '--o', '-*', '-+', '-^', '-p', '-o', '-x', '-d', '-s', '-h'};
index_fu = randperm(length(yang_fu3), 1);
plot(gca, 2:cluster_max, sc_coefficient1, yang_fu3{1, index_fu}, 'Color', color_all(randperm(length(color_all), 1), :), 'LineWidth', 2, 'MarkerSize', makesize)
