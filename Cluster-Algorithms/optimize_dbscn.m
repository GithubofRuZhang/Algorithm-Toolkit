function [Mdl,fMin,Convergence_curve]  = optimize_dbscn(data_O,data_Z,num_pop,num_iter,method_mti)  
 ...
 elseif strcmp( method_mti,'SO蛇优化算法')==1  
  ...
  elseif strcmp( method_mti,'WSO白鲨优化算法')==1    
   ...
   box off 
%   distance_str={'precomputed','euclidean','cityblock','cosine','chebychev','minkowski'};
%   disp([method_mti, '优化dbscn:   ',"  半径范围epsilon:",num2str((pos(1))),"  点个数minpts:",num2str(round(pos(2))),'  distance: ',distance_str{1,round(pos(3))}]) 
 disp([method_mti, '优化dbscn:   ',"  半径范围epsilon:",num2str((pos(1))),"  点个数minpts:",num2str(round(pos(2)))]) 
end
function [fitness_value,index]=fit_dbscn(pop,data_O,data_Z)
     distance_str={'precomputed','euclidean','cityblock','cosine','chebychev','minkowski'};
%      disp([pop(1),round(pop(2))])
     if isnan(pop)
         pop(1)=1;
         pop(2)=1;
     end
     index = dbscan(data_Z,pop(1),round(pop(2)));  
    fitness_value=-mean(silhouette(data_O,index'));
    if(isnan(fitness_value))
        fitness_value=1;
    end
end
