clc;clear;close all;	
load('NNF_KMeans_DBO.mat')	
	
data=G_out_data.data_process;    % Data after overall processing G_out_data.data_O Before processing data	
jiangwei_num=G_out_data.jiangwei_num;    % Set the number of dimensions to reduce	
...
kuang=G_out_data.kuang;   % Selection of frame	
...
if(length(get_legend_str)<yang_num)	
    for i=length(get_legend_str):yang_num	
        get_legend_str{1,i}=['Class',num2str(i)];	
    end	
end	
...
disp_str=['Cluster numbers from 2 to ',num2str(cluster_max),' with silhouette coefficients respectively:'];	
disp(disp_str)	
disp(sc_xishu1)	
figure	
