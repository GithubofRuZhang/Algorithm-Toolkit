function [Mdl,fMin,Convergence_curve]  = optimize_kshape(data_O,data_Z,num_pop,num_iter,method_mti,cluster_limits)  
...
%   distance_str={'sqeuclidean','cityblock','cosine'};
  disp([method_mti, '优化 kshape:   ',"cluster_num:",num2str(round(pos(1)))]) 
end
function [fitness_value,index]=fit_kmeans(pop,data_O,data_Z)
%      distance_str={'sqeuclidean','cityblock','cosine'};
     [index,~] = kShape(data_Z,round(pop(1)));  
...
% DBO_DBO(pop,M,c,d,dim,fobj,data_O,data_Z)
