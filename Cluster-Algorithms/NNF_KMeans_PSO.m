% Clearing command window, workspace variables, and closing all figures
clc;
clear;
close all;

% Loading data from 'NNF_KMeans_PSO.mat'
load('NNF_KMeans_PSO.mat');

% Preparing the data
data = G_out_data.data_process;  % Processed data after overall processing
jiangwei_num = G_out_data.jiangwei_num;  % Setting the number of dimensions for dimensionality reduction
Z = nnmf(data, jiangwei_num);  % Non-negative matrix factorization (NNMF)

% Visualizing high-dimensional data using t-SNE (t-distributed Stochastic Neighbor Embedding)
data_get = tsne(data);  % Default reduction to two dimensions
...
label_distance = G_out_data.label_distance;  % Determining distance metric for clustering
cluster_num = G_out_data.cluster_num;  % Determining number of clusters

% Performing K-means clustering
[index, center] = kmeans(Z, cluster_num, 'Distance', label_distance);

% Calculating silhouette coefficient
sc_xishu = mean(silhouette(data, index'));

% Finding unique cluster indices
a = unique(index);
yang_num = length(a);
C = cell(1, length(a));
for i = 1:length(a)
...
for i = 1:yang_num
    ...
    plot3(data_get(:, 1), data_get(:, 2), data_get(:, 3), fu_str{1, i}, 'Color', color_all(i, :), 'LineWidth', Line_Width(1), 'MarkerSize', makesize);
    hold on;
end
...
% Performing K-means clustering for different cluster numbers
...
% Displaying silhouette coefficients for different cluster numbers
disp_str = ['The silhouette coefficients for cluster numbers 2 to ', num2str(cluster_max), ' are as follows:'];
disp(disp_str);
disp(sc_xishu1);
