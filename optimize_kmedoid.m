function [Mdl,fMin,Convergence_curve]  = optimize_kmedoid(data_O,data_Z,num_pop,num_iter,method_mti,cluster_limits)  
 ...
 elseif strcmp( method_mti,'SO蛇优化算法')==1  
  ...
  disp([method_mti, '优化 kmedoid:   ',"cluster_num:",num2str(round(pos(1))),'  distance: ',distance_str{1,round(pos(2))}]) 
end
function [fitness_value,index]=fit_kmedoid(pop,data_O,data_Z)
     distance_str={'sqeuclidean','cityblock','cosine'};
     [index,~] = kmedoids(data_Z,round(pop(1)),'Distance',distance_str{1,round(pop(2))});  
